...
To configure VSCode to insert `;` at the end of every line of code, press `Ctrl+,` or `Command+,` to go to setting in VScode. Search "JavaScript semicolon" and under `JavaScript>format:semicolon`, choose `insert`.

`$trigger` is another built-in utility $render provides. [Learn more](https://render-docs.vercel.app/utils/$trigger).

[Click to check]() the common errors in using `$render`.