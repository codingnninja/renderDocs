...
`$trigger` is used to call an function when user interact with certain elements. That means you can use it to trigger an action when an interaction like click, hover and so on happens.
...
<Callout type="info" emoji="ℹ️">
  Note: always ensure you use `stringify` with single or double quotes.
</Callout>
