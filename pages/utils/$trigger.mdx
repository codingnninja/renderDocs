...
```js
$trigger(function, selectors, data?optional)
```

It takes three parameters -- `function utility`, `element selector(s)` and `data`.
...
```js copy
$trigger(${function}, selectors, data);
```
...
You need to `$trigger` instead for consistency. You can use a JavaScript function only at almost everywhere you need `$trigger` and it will work in a `script` tag. The problem is it won't work in esModules as the functions won't be accessible.

Use `$trigger` with template literals to hydrate such a function into the DOM so that it will always be accessible. Additionally, `$trigger` allows you to pass elements and data to the function.