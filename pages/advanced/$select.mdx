...
It is recommended to use `$select` with `const` until you need to re-assign a variable. Then, you can select such a variable separately `$select` and `let`.

```js copy
const [audio, posts, comments] = $select('#audio, .post, .comment');
```

So when we need to re-assign the audio element, we can write the selection.
