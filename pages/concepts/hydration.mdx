...
```
You don't need to wrap a function in template literals `${toggle}` while working with a script tags because all functions are accessible in a `script` tag by default. You only need it in esModules.
