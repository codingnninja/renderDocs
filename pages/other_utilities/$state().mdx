...
actions performed by one another. When your components have `one-to-many` or `many-to-many` relationships, you probably need a global state so that changes from components are centralized.

* When a changes in component, affects many other components, it will be hard to be calling each of the components one by one, instead; the changed properties can be centralized so that all affected components access it or it fires all subscribed components.
